...
- (instancetype)init
{
    ...
    if (self) {
        ...
        NSString *callKitAppIconName = [ALAudioVideoSettings getCallKitAppIconName];
        if (callKitAppIconName) {
            UIImage *image = [UIImage imageNamed:callKitAppIconName inBundle:NSBundle.mainBundle compatibleWithTraitCollection:nil];
            if (image) {
...
-(void)endActiveCallVCWithCallReason:(CXCallEndedReason)reason
    ...
    if (self.activeCallModel &&
        ...
        if (self.callListModels.count > 0 &&
            ...
            self.callListModels[callUUID.UUIDString] = callModel;
        }
...
-(void)presentCallVCWithCallUUIDString:(NSUUID *)callUUID
    ...
    if (self.callListModels.count > 0 &&
        ...
        UIStoryboard* storyboard = [UIStoryboard storyboardWithName:@"AudioVideo" bundle:[NSBundle bundleForClass:ALAudioVideoCallVC.class]];
    ...
    [self sendMessageAndEndActiveCallWithCompletion:^(NSError *error) {
        if (error) {
            ...
            if (self.callListModels.count > 0 &&
                ...
                callModel.unansweredCallTimerActive = NO;
                self.callListModels[[action.callUUID UUIDString]] = callModel;
            }
